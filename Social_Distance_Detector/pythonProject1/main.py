...
def getMousePts(e, x, y, flags, param):
    global mouse_pts
...
def calSocialDist(vid_path, net, output_dir, ln1):
    ct = 0
    ...
    output_movie = cv2.VideoWriter("./output/distancing.avi", fourcc, fps, (width, height))
    bird_movie = cv2.VideoWriter("./output/bird_eye_view.avi", fourcc, fps,
    ...
    while True:
        ...
        if ct != 0:
            ...
            cv2.imwrite(output_dir + "frame%d.jpg" % ct, img)
            cv2.imwrite(output_dir + "bird_eye_view/frame%d.jpg" % ct, bird_image)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-O', '--output_vid', action='store', dest='output_vid', default='./output/',
    ...
    calSocialDist(values.video_path, net_yl, output_dir, ln1)
