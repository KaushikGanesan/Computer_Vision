...
def bird_eye_view(frame, distances_mat, bottom_points, s_w, s_h, risk_count):
    h = frame.shape[0]
    w = frame.shape[1]
    ...
    blank_image = np.zeros((int(h * s_h), int(w * s_w), 3), np.uint8)
    blank_image[:] = white
    r = []
    g = []
    y = []
    for i in range(len(distances_mat)):
        ...
        if distances_mat[i][2] == 0:
            ...
            blank_image = cv2.line(blank_image, (int(distances_mat[i][0][0] * s_w), int(distances_mat[i][0][1] * s_h)), (int(distances_mat[i][1][0] * s_w), int(distances_mat[i][1][1] * s_h)), red, 2)
    ...
    for i in range(len(distances_mat)):
        ...
        if distances_mat[i][2] == 1:
            ...
            blank_image = cv2.line(blank_image, (int(distances_mat[i][0][0] * s_w), int(distances_mat[i][0][1] * s_h)), (int(distances_mat[i][1][0] * s_w), int(distances_mat[i][1][1]* s_h)), yellow, 2)
    ...
    for i in bottom_points:
        blank_image = cv2.circle(blank_image, (int(i[0] * s_w), int(i[1] * s_h)), 5, green, 10)
    for i in y:
        blank_image = cv2.circle(blank_image, (int(i[0] * s_w), int(i[1] * s_h)), 5, yellow, 10)
    for i in r:
        blank_image = cv2.circle(blank_image, (int(i[0] * s_w), int(i[1] * s_h)), 5, red, 10)

    return blank_image


def social_distancing_view(frame, dMat, boxes, risk_count):
